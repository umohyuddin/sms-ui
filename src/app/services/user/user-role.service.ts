...
export class UserRoleService {
 ...
 constructor(
  ...
  getUserRole(): Observable<UserRole[]> {
    ...
    return this.http.get<ApiResponse<UserRole>>(ApiConfig.getUserRoles).pipe(
  ...
  createUserRole(role: UserRole): Observable<string> {
    ...
    console.log(payLoad);
    return this.http.post<ApiResponse<{ message: string }>>(ApiConfig.createUserRole, payLoad).pipe(
      map((res) => res.data[0].attributes.message ),
      catchError((error) => {
        if (error.status === 401 && isPlatformBrowser(this.platformId)) {
          this.authService.logout();
        }
        return throwError(() => new Error(error.error?.message || 'Failed to create user role'));
      })
    );
  }
  ...
  updateUserRole(role: UserRole): Observable<string> {
    ...
    const payLoad = createPayload(role);
    console.log(payLoad);
    return this.http.put<ApiResponse<{ message: string }>>(ApiConfig.updateUserRole, payLoad).pipe(
      map((res) => res.data[0].attributes.message ),
      catchError((error) => {
        if (error.status === 401 && isPlatformBrowser(this.platformId)) {
          this.authService.logout();
  ...
  deleteUserRole(role: UserRole): Observable<string> {
    ...
    return this.http.delete<ApiResponse<{ message: string }>>(`${ApiConfig.deleteUserRole}/${role.id}`).pipe(
      map((res) => res.data[0].attributes.message ),
      catchError((error) => {
        if (error.status === 401 && isPlatformBrowser(this.platformId)) {
          this.authService.logout();
        }
        return throwError(() => new Error(error.error?.message || 'Failed to delete user role'));
      })
    );
  }
}
