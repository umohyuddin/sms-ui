...
export class InstituteService {
  ...
  getAllInstitute(): Observable<Institute[]> {
    ...
    return this.http.get<ApiResponse<Institute>>(ApiConfig.getAllIstitutes).pipe(
  ...
  createIstitute(institute: Institute): Observable<string> {
    ...
    return this.http.post<ApiResponse<{ message: string }>>(ApiConfig.createIstitute,payLoad).pipe(
  ...
  updateIstitute(institute: Institute): Observable<string> {
    ...
    const payLoad = createPayload(institute);
    return this.http.put<ApiResponse<{ message: string }>>(ApiConfig.updateIstitute, payLoad).pipe(
      map((res) => res.data[0].attributes.message),
      catchError((error) => {
        if (error.status === 401 && isPlatformBrowser(this.platformId)) {
          this.authService.logout();
  ...
  deleteIstitute(institute: Institute): Observable<string> {
    ...
    return this.http.delete<ApiResponse<{ message: string }>>(`${ApiConfig.deleteIstitute}/${institute.instituteId}`).pipe(
      map((res) => res.data[0].attributes.message),
      catchError((error) => {
        if (error.status === 401 && isPlatformBrowser(this.platformId)) {
          this.authService.logout();
