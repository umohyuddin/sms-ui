...
export class FeeCatalogManagementService {
  ...
  getFeeCatalogById(id: string): Observable<any> {
    return this.http.request(HTTP_METHOD.GET, `${this.baseUrl}${API_ENDPOINTS.FEE.FEE_CATALOG.GET_BY_ID(id)}`, { observeResponse: true });
  }
  ...
  saveFeeCatalog(id: string | null, payload: any): Observable<any> {
    ...
    const url = isUpdate ? `${this.baseUrl}${API_ENDPOINTS.FEE.FEE_CATALOG.UPDATE(id)}` : `${this.baseUrl}${API_ENDPOINTS.FEE.FEE_CATALOG.CREATE}`;
